...
import { Calendar, Users, DollarSign, MapPin, CheckCircle, Clock, AlertCircle } from 'lucide-react';

// --- COMPONENT PHỤ CHO HERO SECTION ---
// Bạn có thể tách component này ra một file riêng (ví dụ: /components/common/CommonHero.jsx) để tái sử dụng
...
const MyTripsPage = () => {
    ...
    if (loading) {
        return <div className="flex justify-center items-center h-screen"><h2>Đang tải danh sách chuyến đi...</h2></div>;
    }
    ...
    if (!user) {
        return (
            <div className="text-center py-24">
                ...
                <p className="text-gray-600 mb-6">Vui lòng đăng nhập để xem các chuyến đi của bạn.</p>
                <Link to="/login" className="btn primary__btn">Đi đến trang đăng nhập</Link>
            </div>
        );
    }
                                                ...
                                                {booking.status === 'approved' && (
                                                    <div className="flex flex-col sm:flex-row justify-between items-center">
                                                        ...
                                                        <Link to={`/checkout/${booking.id}`}>
                                                            <button className="btn primary__btn w-full sm:w-auto">Thanh toán ngay</button>
                                                        </Link>
                                                    </div>
                                                )}
                                                {booking.status === 'pending_approval' && (
                                                     <p className="text-yellow-700 font-medium text-sm text-center">Chúng tôi đang xem xét yêu cầu của bạn và sẽ sớm liên hệ.</p>
                                                )}
                                                {booking.status === 'confirmed' && (
                                                     <p className="text-sky-700 font-medium text-sm text-center">Chuyến đi đã được xác nhận. Chúc bạn có một hành trình tuyệt vời!</p>
                                                )}
                                                 {booking.status === 'rejected' && (
                                                     <p className="text-red-700 font-medium text-sm text-center">Rất tiếc, yêu cầu của bạn đã bị từ chối. Vui lòng liên hệ để biết thêm chi tiết.</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
